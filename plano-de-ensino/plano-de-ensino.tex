...
\begin{enumerate}
	...
	\item participar, durante o \emph{período de aulas} do calendário universitário, de no mínimo três \textit{rounds} com \textit{rating}, ou seja, de \textit{rounds} que são registrados no perfil do competidor.\\
...
\indent Sejam $S_1,S_2,...,S_n$ os \textit{scores} (\textit{score} não é \textit{rating}!) de um aluno nos $n$ \textit{rounds} registrados em seu perfil no período de aulas. Se $n \geq 3$, então
